...
public class JobDAO
{
    ...
    public Map<String, Long> importantSkills(){

        //create list of jobs
        List<Job> jobList = getJobList();

        //create List of Job Skills
       /* List<String> jobSkillsList = jobList.stream()
                .map(Job::getSkills)
                .map(skill -> skill.split(","))
                .flatMap(Arrays::stream)
                .map(skill -> skill.toLowerCase().trim())
                .collect(toList());
        jobSkillsList.forEach(System.out::println);*/

        //create map of Job Skills & it's count in the file
        Map<String, Long> jobSkills = jobList.stream()
                .map(Job::getSkills)
                .map(skill -> skill.split(","))
                .flatMap(Arrays::stream)
                .map(skill -> skill.toLowerCase().trim())
        ...
        //jobSkills.entrySet().forEach(System.out::println);
    ...
    public List<Job> getJobList() {
        ...
        for (Tuple t : jobDataFrame.stream().collect(toList())) {
            Job job = new Job();
